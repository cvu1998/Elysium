...
namespace Elysium
{
    ...
    {
        Matrix dL_wrt_dO;
        std::vector<Matrix> preActivations(m_Layers.size());
        std::vector<Matrix> activations(m_Layers.size());

        if (!m_Layers[0]->forward(inputs, preActivations[0], activations[0]))
        {
...
;
        ...
        for (size_t epoch = 1; epoch <= epochs; ++epoch)
        {
            ...
            if (Verbose || epoch == epochs) ELY_INFO("Epoch: {0} Training Loss: {1}", epoch, loss / (float)itr);
        }
    }
    ...
    void Model::predict(const Matrix& inputs, Matrix& predictions)
    {
        ...
        Matrix lastLinear;
        std::vector<Matrix> preActivations(last);
        ...
        for (size_t i = 1; i < last; ++i)
            m_Layers[i]->forward(activations[i - 1], preActivations[i], activations[i]);
        m_Layers[last]->forward(activations[last - 1], lastLinear, predictions);
    }
    ...
    float Model::score(const Matrix& inputs, const Matrix& targets, Matrix& predictions)
    {
        ...
        for (size_t i = 1; i < last; ++i)
            m_Layers[i]->forward(activations[i - 1], preActivations[i], activations[i]);
        return m_Layers[last]->calculateLoss(activations[last - 1], targets, LossFunction, lastLinear, predictions, loss);
    }
