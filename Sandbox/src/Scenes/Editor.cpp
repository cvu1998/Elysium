...
{
    Elysium::FramebufferSpecification specification;
    specification.Width = width;
    specification.Height = height;
    m_Framebuffer = std::make_unique<Elysium::Framebuffer>(specification);
...
Editor::~Editor()
{
    ...
    Elysium::Render::setViewport(0, 0, m_WindowWidth, m_WindowHeight);
}

bool Editor::isWithinBounds(Elysium::Vector2 position, Elysium::Vector2 bottom, Elysium::Vector2 top)
{
...
Elysium::Vector2 Editor::getCursorPosition()
{
...
void Editor::onUpdate(Elysium::Timestep ts)
{
    if (m_ViewportFocus)
    ...
    {
        m_SameCurrent = true;
        Elysium::Vector2 size = m_Current->Size * 0.5f;
    ...
    if (Elysium::Input::isMouseButtonPressed(ELY_MOUSE_BUTTON_1))
    {
        ...
        for (Quad& q : m_Data.Quads)
        {
            Elysium::Vector2 size = q.Size * 0.5f;
    ...
    if (m_CameraController.getBoundsWidth() * m_CameraController.getBoundsHeight() <= 5000.0f)
    {
        ...
        for (float bottom = bottomBound; bottom < topBound; bottom++)
        {
            ...
            {
                Elysium::Renderer::drawLine({ left, bottom, 0.0f }, { left + 1.0f, bottom, 0.0f }, { 0.5f, 0.5f, 0.5f, 1.0f });
                Elysium::Renderer::drawLine({ left, bottom, 0.0f }, { left, bottom + 1.0f, 0.0f }, { 0.5f, 0.5f, 0.5f, 1.0f });
            }
        }
    }
    for (Quad& q : m_Data.Quads)
    {
        Elysium::Renderer::drawQuadWithRotation(q.Position, q.Size, glm::radians(q.Rotation), q.Color);
    }
    Elysium::Renderer::endScene();
    m_Framebuffer->Unbind();

    Elysium::ImGuiUtility::createImGuiDockspace([&]()
    {
        if (ImGui::BeginMenuBar())
        {
            if (ImGui::BeginMenu("File"))
            {
                ...
                {
                    std::string filepath = std::move(Elysium::Application::openFile("Elysium Scene (*.elysium)\0*.elysium\0"));
                    if (!(filepath.empty()))
                    {
                        m_Data.Quads.clear();
                        m_Current = nullptr;
                ...
                {
                    std::string filepath = std::move(Elysium::Application::saveFile("Elysium Scene (*.elysium)\0*.elysium\0"));
                    if (!(filepath.empty()))
        ...
        Elysium::ImGuiUtility::drawImGuiLabelWithColumn("Camera Translation Speed", [&]()
            {
        ...
        if (m_Current && m_SameCurrent)
        {
            ...
            Elysium::ImGuiUtility::drawImGuiLabelWithColumn("Tag", [&]()
                {
                    ImGui::InputText("##", quadTag, 64);
                    m_Current->Tag = quadTag;
                });
            Elysium::ImGuiUtility::drawImGuiLabelWithColumn("Position", [&]()
                {
            ...
            Elysium::ImGuiUtility::drawImGuiLabelWithColumn("Rotation", [&]()
                {
                    ImGui::SliderFloat("##", &(m_Current->Rotation), 0.0f, 360.0f);
                });
            Elysium::ImGuiUtility::drawImGuiLabelWithColumn("Size", [&]()
                {
                    ImGui::PushMultiItemsWidths(2, ImGui::CalcItemWidth());
            ...
            Elysium::ImGuiUtility::drawImGuiLabelWithColumn("Color", [&]()
                {
        ...
        if (m_ViewportSize != *((Elysium::Vector2*)&viewportPanelSize))
        {
...
void Editor::onEvent(Elysium::Event& event)
{
    ...
    dispatcher.Dispatch<Elysium::MouseButtonPressedEvent>(BIND_EVENT_FUNCTION(Editor::onMousePressedEvent));
    dispatcher.Dispatch<Elysium::WindowResizeEvent>(BIND_EVENT_FUNCTION(Editor::onWindowResizeEvent));
