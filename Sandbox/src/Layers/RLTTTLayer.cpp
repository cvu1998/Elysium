#include "RLTTTLayer.h"

#include "Game/Systems.h"

RLTTTLayer::RLTTTLayer(bool* runConnect4, unsigned int width, unsigned int height) : Layer("Connect4"),
...
RLTTTLayer::~RLTTTLayer()
{
    m_Agent.savePolicyToFile("res/AI/TTTPolicy.rl");
}

void RLTTTLayer::addAction(Elysium::Vector2 position, size_t index)
{
...
void RLTTTLayer::updateAgent(float gameOverReward, float defaultReward)
{
...
bool RLTTTLayer::isWithinBounds(Elysium::Vector2 position, float x1, float y1, float x2, float y2)
{
...
void RLTTTLayer::onUpdate(Elysium::Timestep ts)
{
    if (*m_RunConnect4)
    {
        ...
        if (m_Turn == 1 && m_MoveCooldown >= 0.0f && m_Random)
        {
        ...
        ImGui::Checkbox("Random Player", &m_Random);
...
void RLTTTLayer::onEvent(Elysium::Event& event)
{
    Elysium::EventDispatcher dispatcher(event);
    dispatcher.Dispatch<Elysium::KeyPressedEvent>(BIND_EVENT_FUNCTION(RLTTTLayer::onKeyPressedEvent));
    dispatcher.Dispatch<Elysium::MouseButtonPressedEvent>(BIND_EVENT_FUNCTION(RLTTTLayer::onMousePressedEvent));
    dispatcher.Dispatch<Elysium::WindowResizeEvent>(BIND_EVENT_FUNCTION(RLTTTLayer::onWindowResizeEvent));
}

bool RLTTTLayer::onKeyPressedEvent(Elysium::KeyPressedEvent& event)
{
...
bool RLTTTLayer::onMousePressedEvent(Elysium::MouseButtonPressedEvent& event)
{
    if (!m_GameOver && m_Turn == 1 && m_MoveCooldown >= 0.0f)
    {
...
bool RLTTTLayer::onWindowResizeEvent(Elysium::WindowResizeEvent& event)
{
