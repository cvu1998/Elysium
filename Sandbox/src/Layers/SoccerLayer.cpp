#include "SoccerLayer.h"

SoccerLayer::SoccerLayer(bool* runSandbox, unsigned int width, unsigned int height) : Layer("Soccer"),
...
void SoccerLayer::onUpdate(Elysium::Timestep ts)
{
    if (*m_RunSandbox)
    {
        if (m_GameOver)
...
void SoccerLayer::onEvent(Elysium::Event& event)
{
    Elysium::EventDispatcher dispatcher(event);
    dispatcher.Dispatch<Elysium::KeyPressedEvent>(BIND_EVENT_FUNCTION(SoccerLayer::onKeyPressedEvent));
    dispatcher.Dispatch<Elysium::WindowResizeEvent>(BIND_EVENT_FUNCTION(SoccerLayer::onWindowResizeEvent));
}

bool SoccerLayer::onKeyPressedEvent(Elysium::KeyPressedEvent& event)
{
    return false;
}

bool SoccerLayer::onWindowResizeEvent(Elysium::WindowResizeEvent& event)
{
